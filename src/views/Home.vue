...
<template>
  <div class="flex flex-col">
    <div
      ...
      <div>
        <p class="mb-2">Do you hear the difference between the words below? Sometimes yes, sometimes no, and sometimes
          there's no difference at all — because they genuinely sound the same.</p>
        <p class="mb-2">This app might not be useful for native speakers or those with an English level of C1 and above.
          But for anyone who recently wondered how to correctly pronounce "beach" versus... well, you know... this is
          for you.</p>
        <p class="mb-2">
          <span class="text-md text-lg"><b>What are Homophones?</b></span>
          They are words that truly sound alike, even though they are distinct words (like two, to, and too).
        </p>
        <p class="mb-3">I've also included a group of Non-Homophones in the app. These are words that sound different,
          but it’s not always obvious how to pronounce them correctly.</p>
      </div>
      <div
    ...
    <div class="flex flex-col md:flex-row justify-center gap-10 items-start">
      <section class="flex flex-col w-full md:w-2/3 lg:w-1/2 bg-cyan-50 text-cyan-800 rounded-lg">
        <h2
          ...
          Homophones Group</h2>
        <div class="flex flex-col mt-3 mb-2 lg:mt-7 lg:mb-5">
          <ul v-for="group in homophonesGroup" :key="group.id"
            ...
            <li v-for="word in group.words" :key="word.id" :disabled="isDisabled" @click="handleSpeak(word.word)"
              class="flex my-2 md:my-3 gap-2 px-3 py-2 border border-cyan-800 rounded-lg cursor-pointer">
      ...
      <section class="flex flex-col w-full md:w-2/3 lg:w-1/2 bg-cyan-50 text-cyan-800 rounded-lg">
        <h2
          class="flex justify-center items-center text-xl w-full h-15 bg-gradient-to-b text-teal-900 from-teal-300 via-teal-400 to-teal-400 rounded-t-lg">
          Non-Homophones Group</h2>
        <div class="flex flex-col mt-3 mb-2 lg:mt-7 lg:mb-5">
          <ul v-for="group in nonHomophonesGroup" :key="group.id"
            class="flex flex-row gap-x-5 items-center justify-center p-y-2 flex-wrap">
            <li v-for="word in group.words" :key="word.id" :disabled="isDisabled" @click="handleSpeak(word.word)"
              class="flex my-2 md:my-3 gap-2 px-3 py-2 border border-cyan-800 rounded-lg cursor-pointer">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
