...
<template>
  <div v-if="trainingStore.finished">
    <div v-if="trainingStore.successfulGroups.length"
      class="flex justify-center items-center flex-col w-ful text-cyan-800 rounded-lg">
      ...
      <div v-for="group in filterSuccessfulGroups" :key="group.groupId" class="flex flex-row gap-2">
        <div v-for="word in group.words" :key="word.word"
          class="flex my-2 md:my-1 gap-2 px-3 py-1 border border-cyan-800 rounded-lg cursor-pointe">
          {{ word.word }}
        </div>
      </div>
    </div>
    <div class="mt-3 mb-5 lg:mt-5 lg:mb-8 text-center flex-col w-ful text-cyan-800 rounded-lg" v-else>You don't have any
